...
- **Espace pro** (mêmes routes sous `/garage` et `/market-space`, contrôleur commun `RegistrationDossierController`) : `PUT profile/legal`, `POST|GET profile/legal/document` (un seul document, le nouveau remplace l'ancien), `POST profile/submit` (409 `invalid_status`, 422 `registration_incomplete` avec `missing_fields`/`missing_legal_fields`). `GET profile` ajoute dans `meta` : `legal_status`, `registration` (`status`, `status_label`, `rejection_reason`, `submitted_at`) et `legal` (RCCM, IFU, NPI, présence du document) — seul endroit où le professionnel lit ses informations légales.
