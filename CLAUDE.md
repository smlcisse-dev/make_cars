...
- **Endpoints publics** (`throttle:10,1` par IP, aucun token renvoyé) : `POST /auth/register/professionnel` (demande + envoi du code, table `pending_professional_registrations`, mot de passe et code hachés), `POST /auth/register/professionnel/{uuid}/verify`, `POST /auth/register/professionnel/{uuid}/resend`. Codes d'erreur : `verification_not_found` (404), `verification_code_invalid` (avec `remaining_attempts`), `verification_locked`, `verification_expired` (422), `resend_too_soon` (429, `retry_after_seconds`). Paramètres dans `config/registration.php`, liens des emails construits depuis `config('app.frontend_url')` (`FRONTEND_URL`).
...
- **Demande** : `POST /{garage|market-space}/profile/reactivation-request` `{ message }` (requis, 2000 caractères max), accessible quel que soit le statut du dossier. Compte non suspendu → 409 `not_suspended` ; demande `pending` déjà présente → 409 `reactivation_already_requested` (une seule demande en attente à la fois) ; sinon création de la demande `pending`.
...
- **Frontend** : bandeau de suspension (`SuspensionBanner`, dans `DashboardShell`) avec bouton « Demander la réactivation » (message obligatoire), état « en cours d'examen » ou refus motivé ; côté admin, badge et filtre « Réactivation demandée » dans la liste, bloc de la demande, boutons « Réactiver » / « Refuser la demande » et historique sur la fiche.
