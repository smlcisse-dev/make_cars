...
class GarageListingTest extends TestCase
{
    ...
    private function suspendedGarage(): Garage
    {
    ...
    public function test_only_approved_and_active_services_appear_on_the_garage_profile(): void
    {
        $garage = $this->approvedGarage();
        $approved = RepairService::factory()->forGarage($garage)->approved()->create();
        RepairService::factory()->forGarage($garage)->create();
        RepairService::factory()->forGarage($garage)->approved()->inactive()->create();

        $response = $this->getJson("/api/mobile/garages/{$garage->id}");

        $response->assertOk()
            ->assertJsonCount(1, 'data.services')
            ->assertJsonPath('data.services.0.id', $approved->id);
    }

    public function test_a_suspended_garage_disappears_from_the_public_listing(): void
    {
        $this->approvedGarage();
        $this->suspendedGarage();
    ...
    public function test_a_suspended_garage_profile_is_not_publicly_viewable(): void
    {
        $garage = $this->suspendedGarage();

        $this->getJson("/api/mobile/garages/{$garage->id}")->assertNotFound();
    }

    public function test_a_suspended_garages_products_stay_in_the_database_for_a_future_reactivation(): void
    {
        $garage = $this->suspendedGarage();
        ...
        $this->assertDatabaseHas('garages', ['id' => $garage->id]);
