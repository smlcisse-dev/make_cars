...
class OrderController extends Controller
{
    ...
    private function authorizeOrder(Request $request, Order $order): void
    {
        ...
        abort_unless($order->sellable_type === $account->getMorphClass() && $order->sellable_id === $account->id, 404);
    }

    /**
     * Commandes du Market Space, stock totalement distinct de celui d'un
     * garage (CLAUDE.md §5, ajout v0.9).
     */
    public function index(Request $request): AnonymousResourceCollection
    {
        $orders = $this->authenticatedMarketSpaceAccount($request)->orders()->with(['lines', 'user'])->latest()->paginate();
