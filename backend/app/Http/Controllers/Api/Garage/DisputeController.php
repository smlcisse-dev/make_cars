...
/**
 * Le garage consulte les réclamations le concernant et peut y répondre via
 * l'espace d'échange dédié — jamais de décision de son côté, réservée à
 * l'admin (CLAUDE.md §5, ajout v0.11).
 */
class DisputeController extends Controller
{
    ...
    private function authorizeDispute(Request $request, Dispute $dispute): void
    {
        ...
        abort_unless($dispute->respondent_type === $garage->getMorphClass() && $dispute->respondent_id === $garage->id, 404);
    }

    public function index(Request $request): AnonymousResourceCollection
    {
        $disputes = $this->authenticatedGarage($request)->disputes()->latest()->paginate();
