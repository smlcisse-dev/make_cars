...
export interface ProductMutationResult {
  product: GarageProduct
  message: string
}

export async function fetchGarageProducts(page = 1): Promise<PaginatedResponse<GarageProduct>> {
...
export async function createGarageProduct(
  ...
  const response = await http.post<ApiEnvelope<GarageProduct>>('/garage/products', formData)
...
export async function updateGarageProduct(
  id: number,
  ...
  const response = await http.post<ApiEnvelope<GarageProduct>>(`/garage/products/${id}`, formData)
...
export async function updateGarageProductStock(id: number, payload: StockPayload): Promise<ProductMutationResult> {
  const response = await http.put<ApiEnvelope<GarageProduct>>(`/garage/products/${id}/stock`, payload)
